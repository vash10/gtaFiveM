    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          TriggerEvent('chatMessage', "", {0, 255, 0}, "^5Appuyer sur E pour ouvrir le menu de Banque");
        end
        atBank = true
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^5Vous ne pouvez pas utilisez le distributeur depuis le véhicule!");
    else
      TriggerServerEvent("bank:deposit", tonumber(amount))
    end
  else
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^5Les depots se font uniquement en banque!");
  end
end)
